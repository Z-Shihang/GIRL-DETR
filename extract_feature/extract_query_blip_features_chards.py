...
v_feat_dir = "../Datasets/charades/blip_refine_video_features/"
q_feat_dir = "../Datasets/charades/blip_refine_text_features/"
...
def encode_video(video_path: str):
    ...
    for i in range(n_frames):
        ...
        video_features.append(features_image.image_embeds_proj[:, 0, :])
...
def save_query_features(batch, batch_result, q_feat_dir, training=True):
    for i, result in enumerate(batch_result):
        qid = batch["qid"][i]
        aug_id = batch["aug_id"][i] if training else 0
        aug = f"_{aug_id}" if aug_id > 0 else ""
        q_feat_path = join(q_feat_dir, f"qid{qid}{aug}.npz")
        
        aligned_text_feat = result.text_embeds_proj[:, 0, :].squeeze()
        
        np.savez_compressed(
            q_feat_path, 
            pooler_output=aligned_text_feat.cpu()
        )
